...
| Algorithm             | Dataset            | Mode                     | Command                                               | Expected result|
|-----------------------|--------------------|--------------------------|-------------------------------------------------------|----------------|
| NAC/EnvelopeNets      | CIFAR10            | Construction             | amla add_task configs/config.nac.construction.json    | 0.25 days|
| NAC/EnvelopeNets      | CIFAR10            | Final network            | amla add_task configs/config.nac.final.json           | 3.3%    |
| NAC/EnvelopeNets      | Imagenet           | Construction             | amla add_task.configs/config.nac.construction.json    | 0.34 days |
| NAC/EnvelopeNets      | Imagenet           | Final network            | amla add_task configs/config.nac.final.json           | 13.3%  |
| ENAS (Macrosearch)    | CIFAR10            | Final network            | amla add_task configs/config.enas-macro.final.json (?)| 3.5% (?) |
| AmoebaNet-A           | CIFAR10            | Final network            | amla add_task configs/config.amoebanet-a.final.json(?)| 2.8% (?)  |
| DARTs                 | CIFAR10            | Final network            | amla add_task configs/config.darts.final.json      (?)| 2.1% (?)  |

The expected result is interpreted differently depending on the mode (construction or final network).
